...
let FindDelimiter (input: string) : string array * string =
    //split input to find custom delimiter in the first line
    let mutable toFindDelimiter : string array = input.Split([|'\n'|], StringSplitOptions.None)

    //get the multiple delimiters from the first line in string array format, deleting empty elements in the array
    let delimiters: string array = toFindDelimiter[0].Replace("//", "").Split([|'[';']'|], StringSplitOptions.RemoveEmptyEntries)
